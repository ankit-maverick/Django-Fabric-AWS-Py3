...
configure_instance = [
  ...
  {"action":"apt",
    "params":["libpq-dev", "nginx", "memcached", "git", "libffi-dev",
      "python3-setuptools", "python3-dev", "build-essential", "python3-pip",
      "python-setuptools", "python-dev", "python-pip"], # install python2.7 packages for supervisor
    "message":"Installing apt-get packages"},

  # List of pypi packages to install using Python3
  {"action":"pip3", "params":["virtualenv", "virtualenvwrapper"],
    "message":"Installing pip packages"},

  # List of pypi packages to install using Python2.7
  {"action":"pip", "params":["supervisor"],
    "message":"Installing pip packages"},
  ...
  {"action":"run", "params":"echo 'export VIRTUALENVWRAPPER_PYTHON=/usr/bin/python3' >> /home/%(SERVER_USERNAME)s/.profile"},
  ...
  # Create virtualevn
  {"action":"run", "params":"mkvirtualenv --python=/usr/bin/python3 --no-site-packages %(PROJECT_NAME)s",
    "message":"Creating virtualenv"},

  # install django in virtual env
  {"action":"virtualenv", "params":"pip3 install Django",
    "message":"Installing django"},

  # install psycopg2 drivers for Postgres
  {"action":"virtualenv", "params":"pip3 install psycopg2",
    "message":"Installing psycopg2"},

  # install gunicorn in virtual env
  {"action":"virtualenv", "params":"pip3 install gunicorn",
  ...
  {"action":"virtualenv", "params":"pip3 install -r %(PROJECT_PATH)s/requirements/common.txt --upgrade"},
  {"action":"virtualenv", "params":"pip3 install -r %(PROJECT_PATH)s/requirements/prod.txt --upgrade"},
  ...
  # Setup supervisor
  {"action":"run", "params":"echo 'export PYTHONPATH=$PYTHONPATH:%(SUB_PROJECT_PATH)s' >> /home/%(SERVER_USERNAME)s/.profile"},
  ...
  {"action":"virtualenv", "params":"python %(SUB_PROJECT_PATH)s/manage.py collectstatic -v 0 --noinput"},
  {"action":"virtualenv", "params":"python %(SUB_PROJECT_PATH)s/manage.py migrate"}
]
